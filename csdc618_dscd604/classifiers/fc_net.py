...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        # Weights/biases for each of the num_layers affine transforms
        ...
        # Scale/shift parameters for batch or layer norm (only on the hidden layers,
        # i.e. all layers except the last affine -> softmax)
    ...
    def loss(self, X, y=None):
        ...
        caches = []          # caches for each {affine - [norm] - relu} block
        dropout_caches = []  # dropout caches (one per hidden layer if used)

        out = X
        # Hidden layers: L - 1 blocks of {affine - [norm] - relu - [dropout]}
        for i in range(self.num_layers - 1):
        ...
        # Final affine layer (no ReLU, no norm, no dropout)
        ...
        # Data loss and gradient on scores
        loss, dscores = softmax_loss(scores, y)
        ...
        # Backprop through the final affine layer
