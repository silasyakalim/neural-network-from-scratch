...
def sgd_momentum(w, dw, config=None):
    ...
    # Velocity is a decaying running mean of the (negative) gradient steps.
...
def rmsprop(w, dw, config=None):
    ...
    # Decaying running mean of squared gradients.
    config["cache"] = (
...
def adam(w, dw, config=None):
    ...
    # Increment the timestep BEFORE using it (per the note above).
    config["t"] += 1
    t = config["t"]
    beta1 = config["beta1"]
    beta2 = config["beta2"]

    # First moment (mean) and second moment (uncentered variance) estimates.
