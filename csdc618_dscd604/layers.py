...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    x, gamma, beta, sample_mean, sample_var, eps, x_norm = cache
    N, D = x.shape
    ...
    dx = dx_norm / np.sqrt(sample_var + eps) + dvar * 2 / N * (x - sample_mean) + dmean / N
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*******
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx_norm = dout * gamma
    dvar = np.sum(dx_norm * (x - sample_mean), axis=0) * -0.5 * (sample_var + eps)**(-1.5)
    dmean = np.sum(dx_norm * (-1/np.sqrt(sample_var + eps)), axis=0) + dvar * np.mean(-2*(x - sample_mean), axis=0)
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    cache = (x, gamma, beta, mean, var, eps, x_norm)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*******
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def layernorm_backward(dout, cache):
    ...
    dx = dx_norm / np.sqrt(var + eps) + dvar * 2 / D * (x - mean) + dmean / D
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*******
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    H_prime = 1 + (Hp - HH) // stride
    W_prime = 1 + (Wp - WW) // stride
...
def conv_backward_naive(dout, cache):
    ...
    N, C, H, W = x.shape
    F, _, HH, WW = w.shape
    N_out, F_out, H_prime, W_prime = dout.shape
...
def max_pool_backward_naive(dout, cache):
    ...
    N, C, H, W = x.shape
    N_out, C_out, H_prime, W_prime = dout.shape
        ...
        for c in range(C):
            for i in range(H_prime):
                for j in range(W_prime):
                    ...
                    # Create a mask for the maximum value in the slice
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    x_reshaped = x_reshaped.transpose(0, 2, 3, 4, 1).reshape(N * C // G * H * W, G)
    ...
    out_reshaped = x_norm.reshape(N, C // G, H, W, G).transpose(0, 4, 1, 2, 3).reshape(N, C, H, W)

    out = gamma * out_reshaped + beta

    cache = (x, gamma, beta, G, gn_param, mean, var, x_norm, x_reshaped)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*******
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def spatial_groupnorm_backward(dout, cache):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    x, gamma, beta, G, gn_param, mean, var, x_norm, x_reshaped_forward = cache
    eps = gn_param.get("eps", 1e-5)
    ...
    dx_norm_reshaped = (dout * gamma).reshape(N, G, C // G, H, W).transpose(0, 2, 3, 4, 1).reshape(N * C // G * H * W, G)

    D_group = C // G
    M = N * D_group * H * W

    dvar = np.sum(dx_norm_reshaped * (x_reshaped_forward - mean), axis=1, keepdims=True) * -0.5 * (var + eps)**(-1.5)
    dmean = np.sum(dx_norm_reshaped * (-1 / np.sqrt(var + eps)), axis=1, keepdims=True) + dvar * np.mean(-2 * (x_reshaped_forward - mean), axis=1, keepdims=True)

    dx_reshaped_group = (dx_norm_reshaped / np.sqrt(var + eps)) + (dvar * 2 * (x_reshaped_forward - mean) / G) + (dmean / G)

    dx = dx_reshaped_group.reshape(N, C // G, H, W, G).transpose(0, 4, 1, 2, 3).reshape(N, C, H, W)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*******
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def softmax_loss(x, y):
    ...
    # will be similar to the softmax loss vectorized implementation 
    # in csdc618_dscd604/classifiers/softmax.py.
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
