...
**Notebooks** (run top to bottom, each is self-contained)
...
requirements.txt
prepare_submission.sh   # packages notebooks + code into a submission zip
...
3. Download the datasets (CIFAR-10 and a 25-image ImageNet validation subset):
   ...
   cd ../..
   ```
4. Launch Jupyter and open any notebook from the repository root:
   ```bash
   jupyter notebook
   ```
